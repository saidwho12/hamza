...
layout (location = 0) in vec2 i_bezier_v1;
layout (location = 1) in vec2 i_bezier_v2;
layout (location = 2) in vec2 i_bezier_c1;
layout (location = 3) in vec2 i_bezier_c2;
layout (location = 4) in int i_bezier_type;
...
void main() {
    ...
    int v_index = quad_indices[gl_VertexIndex];
    vec2 pos = quad_vertices[v_index];

    switch (i_bezier_type) {
        case LINE: {
            vec2 v1 = i_bezier_v1, v2 = i_bezier_v2;
            vec2 mi = min(v1,v2);
            vec2 ma = max(v1,v2);
        ...
        case QUADRATIC_BEZIER: {
            vec2 mi,ma;
            bezier2_bbox(i_bezier_v1,i_bezier_c1,i_bezier_v2,mi,ma);
            mi -=max_sdf_distance;// push_constant.max_sdf_distance;
            ma +=max_sdf_distance;// push_constant.max_sdf_distance;
            pos = mi + pos * (ma-mi);
            break;
    ...
    f_pixel_coord = pos;
    f_bezier_v1 = i_bezier_v1;
    f_bezier_v2 = i_bezier_v2;
    f_bezier_c1 = i_bezier_c1;
    f_bezier_c2 = i_bezier_c2;
    f_bezier_type = i_bezier_type;
